...
dimensions = [390*0.01384*1,195*0.01384]
params = {'backend': 'ps',
          'axes.labelsize': 10,
          'text.fontsize': 10,
          'legend.fontsize': 8,
...
for run in range(1,6):
  ...
  ax.errorbar(xs,list(map(lambda x: x/1000, data['distance_from_centre']['run'+str(run)]['meas'])), xerr=None, yerr=0.546703/2, barsabove=True, fmt=None, capsize=2)
...
# ax = plt.gca()
# plt.grid()
# plt.ylabel('Displacement mid-point (mm)')
# plt.xlabel('Tracking feature pair')
# plt.tight_layout()
# plt.savefig("row_tracking.pdf")
...
# xs = range(1,len(data['distance_from_centre']['run1']['meas']))
...
# for run in range(1,6):
#   print(run)
#   ax.plot(xs,data['distance_from_centre']['run'+str(run)]['meas'][0:40])
#   ax.plot(xs,data['distance_from_centre']['run'+str(run)]['max'][0:40])
#   ax.plot(xs,data['distance_from_centre']['run'+str(run)]['min'][0:40])
# # ax.bar(range(1, len(datasource)+1), datasource, color='b', edgecolor="none", align="center", width=0.7)
# # ax.set_xlim(4.0, 4.5)
# ax.set_ylim(-800,800)
...
# plt.xlabel('Tracking feature pair')
# plt.tight_layout()
# plt.savefig("row_tracking_average.pdf")
