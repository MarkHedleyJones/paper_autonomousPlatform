...
\label{sect:autonomous}
    ...
    Both algorithms were developed on smaller, commercially available, test platforms while the presented platform was being fabricated.
    ...
    While it would be possible to combine the approaches, only the lidar based method was adapted for use on the presented platform.
...
\label{sub:testing}
  ...
  \subsection{Row Following}
    ...
    The algorithm used was similar to that reported in \cite{Bell2016}, but with the following alterations.
    ...
    This involved first filtering out points that were expected to represent the ground and using them to create a smooth surface.
    ...
    Once there were no post/trunk pairs further than \SI{2.5}{\meter} in front of the vehicle, it stopped.
    ...
    The vertical grey lines link the post/trunk pairs that were used to calculate row mid-points.
    ...
    In this analysis we assume that the generated SLAM map is perfectly accurate, which is not the case in reality.

    Figure~\ref{fig:row_following_repeatability} then compares each trajectory to the average trajectory.
