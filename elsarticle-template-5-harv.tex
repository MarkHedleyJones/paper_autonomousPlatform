...
\label{sect:design}
    ...
    \label{sub:drive}
        ...
        At the output of the gearbox the torque jumps to \SI{816}{\newton\meter} while the angular velocity drops to \SI{75}{rev\per\minute}; giving the platform a top speed of \SI{10.3}{\kilo\meter\per\hour}.
    ...
    \label{sect:architecture}
        ...
        In this configuration, the analog inputs fed into a master controller are relayed to a second controller over a CAN interface.
...
\label{sub:testing}
  ...
  \subsection{Bin Lifter}
    ...
    The vehicle was driven for \SI{300}{\meter} with the load, which included an \SI{8}{\degree} incline for \SI{30}{\meter}.
  ...
  \subsection{Row Following}
    Row following repeatability testing was done by allowing the platform to self-drive through a row, from the same starting point, five times.
    The algorithm used was similar to that reported in \cite{Bell2016}, but had a couple of minor differences.
    It used only a multi-layer lidar (Velodyne VLP-16) and IMU (LPMS-USBAL2), but made no attempt to avoid obstacles.
    It's end-of-row detector was based on the vehicle's proximity to the row's last pair of posts instead of detecting the presence of canopy above.
    ...
    %% TODO;
    % It would be quite good to do some analysis on the drive path and show how the path tracks a row centre line and show the result in a graph.
    % Update the writing of the repeatability measurements to match the more recent tests (including updating or maybe replacing figure 16).
    ...
    \begin{figure}[htb]
        \centering
        \includegraphics{imgs/slam/end_positions.pdf}
        \caption{
    ...
    An interesting point occurs approximately one-third of the way through each run where each of the paths diverge briefly.
    The location is visible in Figure \ref{fig:row_following_paths} in the enlarged box.
    Replaying the captured sensor data through the row-following algorithm revealed a cluster of transient points from a patch of weeds near a trunk.
    These points interfered with the localisation of the trunk due to their close proximity.
    The algorithm uses the positions of these posts to determine row the centre-points, where some instability was evident at this location.
...
\label{sect:discussion}
    ...
    The reported platform meets the requirements and has proved its usefulness during three pollination and harvesting seasons.
    During these operations the vehicle was operated under manual control, because of the need to drive so close to row boundaries.
    The width of the vehicle and modules meant it is necessary to perform two passes down each row to access the full canopy area.

    The results from testing of navigation algorithms indicate that the multi-layer lidar with wheel encoder feedback are sufficient for row turning tasks, while an IMU and multi-layer lidar are sufficient for row following tasks.
    ...
    Future work is to combine the lidar, imu, and encoder based odometry outputs into a single measurement that combines the benefits of each.
    ...
    The repeatability of row following based navigation was acceptable, however the algorithm lacked the ability to avoid obstacles.
    The ability to avoid obstacles will be required before the vehicle can be used commercially.
    Observationally, detecting the end of row based on the vehicle's proximity to the last posts proved more reliable than detecting the presence/absence of canopy above.
    ...
    The authors suggest a pack voltage of \SI{48}{\volt} as an alternative during vehicle development as it bears a reduced risk of injury from electric shock.
    ...
    The cost benefit of using a lower gauge of cable did not outweighed the extra safety procedures required during development.
    ...
    The model also proved useful for visualising the state of the vehicle and sensor outputs, using Rviz, in real-time or when replaying recorded sensor data.

\section{Conclusion}
    This work presented a platform designed specifically for autonomously transporting task-specific modules through pergola style kiwifruit orchards.
    The platform proved capable of carrying twice the mass of platforms previously reported in the literature.

    Multi-layer lidar proved to be the most versatile sensor for orchard based navigation owing to its wider field-of-view and being simpler to interpret that image data.
    ...
    Row following based only on lidar and an IMU
