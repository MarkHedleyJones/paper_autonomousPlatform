...
\begin{abstract}
    Horticultural robots designed for in-field use usually require a means of transport around orchards or farms.
    A common approach is to directly integrate a drive system, at the expense of increasing overall complexity.
    Alternatively, such robots can be modularised and attached to general purpose platforms.
    ...
    Examples of related modules are robotic kiwifruit harvesters and kiwifruit pollination modules.
    ...
    Combined with a method of automatically turning between rows, the vehicle traversed over \SI{10}{\kilo\meter} of orchard rows unassisted.
\end{abstract}
...
\label{sect:intro}
    ...
    Details of modules developed for use on the vehicle have been published separately \citep{Williams2019a, Williams2019b}.
    ...
    This differs from a harvesting module, which advances a set distance between periods of being stationary.
    The platform must be able to self-drive in a way appropriate for each.
...
\label{sect:review}
        ...
        While their purpose built experimental vehicle proved capable of row following and targeted spraying, it was not modular.
        ...
        While it had a basic appearance, it contained most of the functionality required by our modularised fruit harvesting and pollination modules.
...
\label{sect:design}
    ...
    \label{sub:steering}
        ...
        The use of a two-wheeled steering system removes the need to develop the ``non-trivial'' control strategies developed by \cite{Bak2004}.
    ...
    \label{sub:power}
        ...
        This configuration allowed the system to control the rate of power generation by commanding braking effort via its CAN interface.
        ...
        An external charging port is also fitted to allow charging of the batteries externally.
        ...
        A connection panel at the front of the module mounting area houses the weather-sealed plugs (Anderson SPEC PAC Series) through which these outputs are accessible.
    ...
    \label{sect:architecture}
        ...
        It too is an x86 based PC running Ubuntu, but also contains two discrete graphics cards (Nvidia GTX 1080Ti).
        ...
        For example, the accelerator and steering inputs are required to be controlled by potentiometers actuated by the vehicle's driver.
...
\label{sect:autonomous}
    ...
    The second method used a single camera combined with a convolutional neural network to segment areas within the image \citep{Bell2017}.
    ...
    \label{sub:rowfollowing}
      ...
      The process involved evaluating groupings of four post/trunks to find those likely to represent adjacent post/trunks on either side of the row -- forming a rectangle.
      ...
      A similar process was then repeated to join the rectangles into a lattice, representing a continuous section of an orchard row.
      ...
      Again using cost functions, possible lattice sections were then evaluated based on the regularity of areas of the rectangle they were made up of, their linearity, and the number of rectangles they contained.
...
\label{sub:testing}
  ...
  \subsection{Turning Between Rows}
    ...
    All but four interventions were due to an imminent collision with a post from this situation.
  ...
  \subsection{Pure Row-following}
    ...
    % TODO{Mark} Add more details of how the row-following algorithm determines the end of row, referring to what I add earlier.
    ...
    That extra spacing places the final post-pair at the boundary of the software-defined cut-off region used to identify post trunk pairs.
...
\label{sect:discussion}
    ...
    % TODO{Mark}: Discuss that there is high localisation uncertainty at the start because we do not use a pre-computed map and because there isn't much sensor information at the commencement of each trial.
    ...
    For row following, a multi-layer lidar algorithm produced paths that were repeatable to within $\pm$\SI{75}{\milli\meter} of each another.
    ...
    The regular and easily detectable structure of a pergola style kiwifruit orchard made generating odometry information directly from lidar scans reliable.
    This data was generated by the SLAM package and removed the need for wheel odometry and an IMU when performing row following.
    As it was based on visual information it was much less prone to drift than the wheel encoders or IMU, although we did not quantify the difference.
    ...
    Inputs of \SI{48}{\volt} are also supported across a wider range of motors, motor controllers, and power converters, but cabling requirements are increased.
    ...
    In the event that the vehicle required repair or maintenance work during a harvesting or pollination season, the modules could be mounted on a trailer and towed behind an ATV until the vehicle was ready.
