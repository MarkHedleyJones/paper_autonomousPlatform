...
\label{sect:design}
    ...
    \label{sub:drive}
        ...
        It uses a front pivoting axle, as shown in Figure~\ref{fig:AMMPChassis}.
...
\label{sub:testing}
  ...
  \subsection{Row Following}
    ...
    Each test run had the vehicle start from a position that had previously been marked out on the grass.
    ...
    This algorithm is different to that used to test the row turning and lacks the ability to avoid obstacles.
    ...
    The algorithm uses the positions of these posts to determine row centre-points, one of which was unstable at this location.

    Wheel odometry provided by the drive motors was not used during row following as signs of slippage were apparent.
    Quick measurements conducted on grass showed an imbalance of approximately \SI{5}{\percent} between the rear wheels depending on the slope angle (which was not measured, but estimated to be about 5 degrees).
...
\label{sect:discussion}
    ...
    Wheel slippage meant that the encoder information from driven wheels was less suitable for long-term odometry than the odometry information provided from SLAM packages processing lidar and IMU data.
    ...
    The repeatability of row following based navigation was acceptable, however the algorithm lacks the ability to avoid obstacles.
    The ability to avoid obstacles will be required for longer term use.
