...
\label{sect:review}
    ...
    %     Sensor combinations for orchard row detection in orchards mostly fall into three categories; camera based, lidar based, or a combination of the two.
    %     The following section summarises a review of row detection efforts in orchards using these sensors.

    %     When evaluating the use of both lidar and camera based row following systems, both \cite{Subramanian2006} and \cite{Scarfe2012} favoured the use of a single plane lidar over camera based systems.
    %     Scarfe cited difficulties in dealing with variability in lighting conditions using camera based systems.
    %     Subramanian et~al.\@ found lidar to be more accurate generally.
    %     Lidar was used by \cite{Barawid2007} to guide a drive-by-wire tractor through an orchard at \SI{0.36}{\meter\per\second} (\SI{1.3}{\kilo\meter\per\hour}).

    %     \cite{Barawid2007} demonstrate the use of data from a single-plane lidar to guide a drive-by-wire tractor through an orchard at \SI{0.36}{\meter\per\second} (\SI{1.3}{\kilo\meter\per\hour}).
    %     \cite{Hansen2011} show the use of a single-plane lidar for vehicle localisation in an orchard.
    %     \cite{Freitas2012} demonstrated off-line obstacle detection of people and bins within rows of an apple orchard using a single plane lidar.
    %     The following year, many of the same authors write about their autonomous vehicle \citep{Bergerman2015}.
    %     It is capable of row following and executing programmed row end turns in orchards using lidar and encoders.

    %     Work using cameras to generate centre lines for row following suggest that cameras alone are unsuitable when the environment becomes complex \citep{He2011,Sharifi2015}.
    %     Improvements to complex lighting conditions can be had when combining traditional cameras with neural networks \cite{Torres2011}.

    %     Previous work suggests two navigation sensor approaches as being suitable for use in orchards.
    %     Lidar saw three of the reported vehicles navigate autonomously through orchard environments, which is encouraging.
    ...
    %     Based on the increased reception requirements, we discount the use of RTK based systems, but still consider the use of general purpose GNSS receivers as a secondary input.
        ...
        % They found that lidar proved more accurate until its data transfer rate became a limiting factor.
        % The 2D camera based approach was favourable after this point.
        ...
        % Second, when combining cameras with neural network based processing, robustness to environmental complexities such as light or clutter was improved.
        % Reports and previous experience suggest traditional image based processing for navigation failed when the scene became complex or significant variations in lighting occurred.
...
\label{sect:autonomous}
    ...
    % Because only minor angle adjustments are required when row following, these differences are considered negligible in this context.
    % Alterations were made to account for the change in pivoting axis, but not for actuation delay.
    ...
    % The lidar is used to determine the presence of canopy.
    % the vehicle is said to be either {\tt in\_row} or {\tt out\_of\_row}.
    % This state variable determines when the algorithm begins executing the row-end turn for both the lidar and camera based approaches.
    ...
    % Manual adjustments are made to the turn-segments based on the vehicle's proximity to obstacles or nearby boundaries during execution.
    ...
    %         \textbf{Row \#}        & 1      & 1       & 1        & 1        & 1       & 2         \\
    %         \textbf{Sequence \#}   & 1      & 2       & 3        & 4        & 5       & 0         \\
    %         % \textbf{State}       & FOLLOW & END     & END      & END      & END & SEARCH \\
    %         \textbf{Speed}         & 1.0    & 0.3     & 0.3      & 0.3      & 0.3     & 0.3       \\
    %         \textbf{End Criteria}  & CANOPY & HEDGE   & DISTANCE & DISTANCE & CANOPY  & DISTANCE  \\
    %         \textbf{End Parameter} & 5      & 2.3     & 2.5      & 2.75     & 5       & 5.1       \\
    %         \textbf{Turn radius}   &  -     &  -      & 2.0      & 1.95     & 1.95    &  -        \\

    %     \end{tabular}
    %     \caption{Row turn profile}
    %     \label{table:turn_profile}
    % \end{table}
...
\label{sect:discussion}
    ...
    The need to drive close to row boundaries was a consequence of limitations with the separately developed modules.
     % as the modules were expected to require only a single pass per row.
...
\section{Conclusion}
    ...
    % Tests suggest either multi-layer lidar or a combination of 2D-camera and neural network based processing are sensible choices as row following sensors.
