...
\label{sect:review}
    ...
    \subsection{Purpose-built Autonomous Vehicles in Agriculture}
        ...
        With the exception of the platform presented by \cite{Scarfe2012}, all of the reviewed platforms are designed around open field crops.
        None were designed for harvesting operations and therefore are not capable of carrying bins.
        Referring back to the statement from \cite{Ruckelshausen2009} that ``the application itself should be of comparably low complexity'', it is easy to see why research thus far has focused on simpler tasks such as inspection or weeding.
        ...
        Significant cost reductions are possible by using controllers designed for vehicle integration by automotive manufacturers as opposed to more general purpose motor controllers.
        While not a physical component, the CAN bus is another such technology borrowed from the automotive industry.
        Most of the more recent platforms reviewed made use of this communication system to some degree.
...
\label{sect:design}
    ...
    \label{sub:power}
        ...
        The engine is capable of producing \SI{16}{\kilo\watt}, where up to \SI{9.6}{\kilo\watt} is converted to electrical power, artificially limited in software, and \SI{4.0}{\kilo\watt} to pneunatic power.

        Electrical generation is done with a brushless AC motor/generator (Heinzmann PMSG-150) and the same model of motor controller used in the drive system.
        The motor is a larger variant of those used for traction and steering, but no gearbox is fitted.
        The controller is configured only to have regenerative braking functionality.
...
\label{sect:discussion}
    ...
    The need to dive close to row boundaries was unexpected as the modules themselves were initially planned to require only a single pass per row.

    The results from prototyped algorithms indicate that the multi-layer lidar and wheel encoder feedback are enough for mapping, localisation, and row following tasks.
    ...
    The two approaches for row following proved reliable, however the method for turning between rows needs improving.
    Future work will focus on enabling the system to plan row-end turns based on sensor data, without the need for a manually created map.
...
\section{Conclusion}
    ...
    In-orchard evaluation of sensors suitable for navigation suggest that multi-layer lidar is a sensible choice as a primary navigation sensor.

    Multi-layer lidar was found to offer reliable data
