...
\label{sect:autonomous}
    ...
    Because posts/trunks in the same row-line are nearer than those of the adjacent row, this approach generated row boundary lines.
    Once each side of the row was determined, linear and angular offsets were calculated between the row's centre and the vehicle.
...
\label{sub:testing}
  ...
  \subsection{Row Following}
    ...
    In this trial the row-following algorithm was similar to that reported in \cite{Bell2016} that used a lidar to detect post/trunks, but with three main differences.

    Firstly, the filtering of incoming point-cloud data was altered such that incoming points were filtered based on their proximity to the ground.
    ...
    That surface was then used to filter the remaining points based on their height above the ground; not so far as to potentially be part of the overhead canopy, but not so close that they may be weeds or related to the ground itself.
    ...
    The process involved a recursive walk around the filtered points to search for squares defined by posts/trunks; a pair on the left boundary and a pair on the right.
    Those squares were evaluated using a cost function to find those having a high degree of symmetry, regular internal angles, and appropriate length of sides,
    A similar process was then repeated to join the squares into chains that represented a continuous section of row.
    At each stage, the squares, and chain proposals were evaluated using cost functions to weigh up and filtered based on minimum acceptable criteria.

    This was repeated for every frame sent from the lidar (approximately \SI{10}{\hertz}).
    The vehicle then steered toward the mid-point of the most distant post/trunk pair in front of itself.
    Secondly, detection of the \textit{out\_of\_row} condition was calculated based the vehicle's proximity to the farthest post/trunk pair in front of the vehicle.
    Once there were no post/trunk pairs further than \SI{2.5}{\meter} in front of the vehicle, the vehicle would stop.
    Thirdly, this algorithm made no attempt to turn between rows.
    ...
    \begin{figure}[htb]
        ...
        \caption{
            Five row-following trials shown with row boundaries overlaid.
    ...
    In this analysis we assume that the generated SLAM map is perfectly accurate.
    ...
    Analysis of the recorded sensor data shows this ambiguity to be the cause of the separation in end-positions between points A and B.
