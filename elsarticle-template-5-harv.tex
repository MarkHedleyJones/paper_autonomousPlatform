...
\begin{abstract}
    ...
    Sensors suitable for autonomous navigation are evaluated in-orchard and tested using prototype navigation algorithms.
...
\label{sect:intro}
    ...
    Development of an automated kiwifruit harvesting system with integrated drive system has previously been published \citep{Scarfe2012}.
    ...
    The systems are generally bulky and have specific geometric requirements dictated by the kiwifruit growing systems.
    ...
    To enable the whole system to work autonomously, the base platform must be able to self-drive in a way appropriate for both modules.
    ...
    The presented vehicle must navigate through kiwifruit orchards by relying only on perception based sensors.
    Other requirements for the vehicle are:
    \begin{enumerate}
    ...
    \begin{figure}[htb]
        ...
        \caption{
            The presented platform driving through a row of pergola style kiwifruit orchard during winter.
        }
        \label{fig:suzy}
    \end{figure}
...
\label{sect:review}
        ...
        Similar to the previous unit presented by \cite{Bak2004}, it featured a gyroscope, RTK-GPS receiver, CAN bus, and four-wheel steering.
        ...
        It was capable of carrying a \SI{150}{\kilo\gram} payload in its dedicated module space; the gross weight of the vehicle presented by \cite{Bak2004}.
        What made BoniRob particularly interesting was its ability to alter its track width by actuating the arms to which its wheels were mounted.
...
\label{sect:design}
    ...
    \label{sub:steering}
        ...
        Both steered wheels have the freedom to rotate \SI{330}{\degree}, artificially limited by software and mechanical stops.
        ...
        Headlands in kiwifruit orchards are sized for tractors with much larger turning radii than that of the platform.
    ...
    \label{sub:drive}
        ...
        The vehicle's three-point suspension system ensures all wheels remain in contact with the ground.
        It uses a front pivoting axle to do this, which is depicted in Figure~\ref{fig:AMMPChassis}.
        As the operating speed for the vehicle is \SI{1.39}{\meter\per\second} (\SI{5.0}{\kilo\meter\per\hour}), the tyres alone were expected to provide sufficient capacity for shock absorption.
    ...
    \label{sub:power}
        ...
        This motor is a larger variant of those used for traction and steering, but without a gearbox.
        ...
        These settings provide all the functionality of a general purpose lithium-ion battery charger, making this a cost effective and versatile charging solution.
    ...
    \label{sect:architecture}
        ...
        Interface adapters, motor controllers, wireless controllers, lidar, and encoders are examples of devices with dedicated interface nodes.
        ...
        Among other things, ROS offers the ability to monitor and record all communication passing through it which can be replayed and interrogated at a later date.
...
\label{sect:sensors}
    ...
    \subsubsection{In-orchard Lidar Evaluation}
        ...
        Similarly this issue arose on concave terrain where the plane intercepted with the ground.
        This situation is depicted in Figure~\ref{fig:concaveSlope}.

        \begin{figure}[htb]
            \centering
            \includegraphics[width=\linewidth]{imgs/concave_slope/concave_slope_v4.pdf}
            \caption{
                On concave slopes the lidar scan plane can intercept with the ground before striking row features.
        ...
        The issue was reduced by the use of a multi-layer lidar and post-processing.
...
\label{sub:testing}
  ...
  \subsection{Drive System}
    ...
    \begin{figure}[htb]
        ...
        \caption{
            Acceleration of the platform on flat ground (red) and up a \SI{3.5}{\degree} incline (blue).
        }
        \label{fig:acceleration}
    \end{figure}

    Acceleration testing saw the platform accelerated from stand-still to a commanded speed of \SI{2.78}{\meter\per\second} (\SI{10}{\kilo\meter\per\hour}) on both flat ground and up a \SI{3.5}{\degree} incline.
    The acceleration was measured using an Inertial Measurement Unit (IMU, LPMS-USBAL2).
    Figure~\ref{fig:acceleration} shows the acceleration profile of the platform in both cases.
    The platform's weight during the test was estimated to be \SI[separate-uncertainty=true]{850(50)}{\kilo\gram}.
    ...
    Based on the specifications of the motors and controllers, there should be little difference in acceleration profile between flat and a \SI{3.5}{\degree} slope,
    however there is a clear deviation.
    % During the flat and climb acceleration tests a peak power output of \SI{2.90}{\kilo\watt} and \SI{2.17}{\kilo\watt} were calculated.
    During the flat acceleration test a peak power of between \SI{684}{\watt} and \SI{770}{\watt}, and torque of between \SI{930}{\newton\meter} and \SI{1046}{\newton\meter} was calculated per wheel.
    During the climb acceleration test a peak power of between \SI{542}{\watt} and \SI{610}{\watt}, and torque of between \SI{779}{\newton\meter} and \SI{876}{\newton\meter} was calculated per wheel.
    The torque calculations suggest the motors are developing their specified output of \SI{816}{\newton\meter}.
    The motor controllers are configured to supply extra torque for short bursts, which could explain why the calculated torque on flat ground is higher than this value.
    The climb test began with the vehicle being held stationary using torque-control, which may have affected the controller's ability to produce the higher peak torque in this case.
    The lower than expected top speed in both cases suggest there are configuration issues with the motor controllers.

  \subsection{Turning Circle}

    Measurement of the vehicle's turning radius were performed at speeds of \SI{1.39}{\meter\per\second} (\SI{5.0}{\kilo\meter\per\hour}) and \SI{2.78}{\meter\per\second} (\SI{10.0}{\kilo\meter\per\hour}) on both dry tarmac and damp grassland.
    These speeds were calculated at the mid-point between the two front wheels.
    Having a wheelbase of \SI{2.67}{\meter}, these speeds of equate to angular velocities of \SI{0.520}{\radian\per\second} and \SI{1.04}{\radian\per\second} respectively.
    ...
    This gives a turning radius of \SI{3.23}{\meter}, \SI{0.05}{\meter} wider than the estimates from kinematic calculations.
  ...
  \subsection{Bin Lifter}
    ...
    Lowering was done by simply opening both ports to atmosphere and allowing the load to descend under its own weight.
    The load was lifted and lowered a total of five times.
    The vehicle was driven for \SI{300}{\meter} with the load, which included an \SI{3.5}{\degree} incline for \SI{30}{\meter}.
    ...
    The shuddering was due to excessive and unbalanced static-friction on each side of the four-bar mechanism.
    This friction was caused by overly tight-fitting sleeve bearings.
  ...
  \subsection{Row Following}
    ...
    It's end-of-row detector was based on the vehicle's proximity to the row's last pair of posts, as opposed to detecting the presence of canopy above.

    Each test was conducted at the vehicle's target operating speed of \SI{1.39}{\meter\per\second} (\SI{5}{\kilo\meter\per\hour}).
    To determine the vehicle's trajectory, recordings of the lidar and IMU were made and run through a SLAM package off-line.
    The SLAM package used was Google Cartographer (version 1.0) used as a ROS package that produced both the map and the vehicle's trajectory information \citep{Hess2016}.
    ...
    Quick measurements conducted on grass showed an imbalance of approximately \SI{5}{\percent} between the rear wheels depending on the slope angle.
    ...
    \begin{figure}[htb]
        ...
        The whiskers represent the maximum deviations in both the positive and negative directions.
        Each trial has a sample size of 40 points.
        }
    ...
    The analysis of the tracking performance is particularly difficult as the positioning of the posts/trunks show obvious signs of deviation, evident in Figure~\ref{fig:row_following_paths_segmented}.
    ...
    Without significant corrections to the data, which would be subjective, the analysis describes more about the linearity of the orchard's row defining features than the path tracking performance.
    ...
    It shows that the worst case repeatability less than $\pm$\SI{75}{\milli\meter}.

    Finally, Figure~\ref{fig:row_following_end_points} shows the end position of each path.
    The path-following algorithm determines the end position based on its proximity to the last post within it's detection range.
    Notice that the last post in each row are spaced slightly further than is usual inside the row.
    This is visible along the left hand side of the SLAM map shown in Figure~\ref{fig:row_following_paths} and in the photo of Figure~\ref{fig:suzy_turning}.
    That extra spacing places the final post pair near the cut-off region used to identify post/trunk pairs.
...
\label{sect:discussion}
    ...
    The reported platform meets the specification requirements and has proved its usefulness during three pollination and harvesting seasons.
    During these operations the vehicle was operated under manual control because of the need to drive close to row boundaries.
    The width of the modules meant it was necessary to perform two passes through each row in order to access the full canopy area.

    Results from navigation tests indicate that multi-layer lidar with wheel encoder feedback are sufficient for row turning tasks.
    For row following, an IMU and multi-layer lidar based algorithm produced paths which were sufficiently repeatable and close to the row's centre-line.
    Wheel slippage meant that the encoder information from driven wheels was less suitable for long-term odometry than the odometry information provided by the SLAM package, which was based on processed lidar and IMU data.
    Future work will combine the lidar, IMU, and encoder based odometry outputs into a single output for longer term accuracy.
    ...
    Observationally, detecting the a row's end-point based on the vehicle's proximity to the last post pair proved more reliable than detecting the presence/absence of canopy above.
    ...
    The authors suggest a pack voltage of \SI{48}{\volt} as an alternative as it bears a reduced risk of injury from shock.
    ...
    However, robotic modules and the bin-lifting mechanism require pneumatic pressure.
    As the air-compressor is belt driven from the petrol engine it is necessary to frequently run the engine.
...
\section{Conclusion}
    ...
    The platform proved capable of carrying twice the mass of platforms previously reported in the literature.
    ...
    Row following based only on lidar and an IMU proved to be repeatable to within $\pm$\SI{75}{\milli\meter}.

    The authors recommend a system voltage of \SI{48}{\volt} for development platforms due to safety and component availability.
