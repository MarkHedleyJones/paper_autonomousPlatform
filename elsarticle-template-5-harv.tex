...
\label{sect:sensors}
    ...
    \subsubsection{In-orchard Camera Evaluation}
        ...
        The Flir camera had a USB2 interface and an average image trasfer time of \SI{65}{\milli\second}.
        ...
        Using two of these 2D cameras it is also possible to build a stereoscopic pair.
        This provides the same functionality of the 3D stereoscopic camera from Intel, but without requiring the infrared projector that prevented it working outdoors.
        Stereo pairs of industrial cameras have since proven useful on modularised harvesting and pollination modules for localising fruit and flowers.
...
\label{sect:autonomous}
    ...
    Specific details and results of each have been published separately.
    ...
    This means that when turning, the a skid-steered platform pivots along a lateral axis passing between the front and rear wheels.
    On the target platform, that axis instead passes through the centre of the rear wheels.
...
\label{sub:testing}
    ...
    After tuning of the row end turn manoeuvres, the platform navigated Block A consecutively 7 times.
...
\section{Conclusion}
    ...
    Multi-layer proved to the most versatile sensor for navigation in general, owing to its wide field of view.
    One particularly useful aspect was the ability to detect the presence of canopy above the vehicle.
    Using a map of manually adjusted row end turns the platform has navigated over \SI{10}{\kilo\meter} of orchard blocks without intervention using only lidar and wheel encoders.
