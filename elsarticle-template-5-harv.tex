...
\label{sect:sensors}
    ...
    \subsubsection{In-orchard Camera Evaluation}
        ...
        Another option is to use a pair of the industrial cameras with stereo matching algorithms.
        This is outside the scope of this work, but is something considered for future work.
...
\label{sect:autonomous}
    ...
    One method used multi-layer lidar to detect trunks and posts and follow a centre-line between them \citep{Bell2016}.
    ...
    This method was used to initiate a row-end turn manoeuvre in the case of both the lidar and camera based approaches.

    \color{red} This is where the row end turn stuff gets added!!!!!!!!!!!!!!! \color{black}

    A row-end turn sees the platform execute a series of manoeuvre which have previously been recorded for the specific row and direction of turn.
    Initially, a template manoeuvre is executed at each row-end and the vehicle is observed while executing the turn.
    Manual adjustments are made to the recorded turn profile based on the vehicle's proximity to obstacles or nearby boundaries during the manoeuvre.
