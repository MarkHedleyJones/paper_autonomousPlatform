...
\label{sect:review}
    ...
    \subsection{Purpose-built Autonomous Vehicles in Agriculture}
        ...
        Similar to the unit presented by \cite{Bak2004}, it featured a gyroscope, RTK-GPS for localisation, a CAN bus for low level communication, and four wheel steering.
        ...
        A \SI{2.8}{\kilo\watt} petrol generator could be mounted to its chassis, additional to its on-board batteries.
        ...
        Many of the platforms reviewed, especially the more recent ones, made use of this communication system.

        There appears to be a trend where platforms designed specifically for open field crops favor four-wheel steering over two wheel steering, with the exception of the Agbot II.
    ...
    \subsection{Sensors for Row Based Navigation in Orchards}
        ...
        Lidar proved more accurate the lidar's data transfer rate became a limiting factor.
...
\section{Platform Design}
    ...
    \label{sect:mechanical}
        ...
        From this, on-board power supplies can continuously deliver \SI{2.8}{\kilo\watt} at \SI{12}{\volt}, \SI{3.8}{\kilo\watt} at \SI{24}{\volt}, and \SI{3500}{\kilo\watt} at \SI{240}{\volt}AC, simultaniously, while driving.
    ...
    \label{sect:architecture}
        ...
        A second computer dedicated to processing sensing data for autonomous navigation is connected to the platform PC via Ethernet.
        ...
        This CAN bus monitoring ensures that the system is automatically shut down in a timely manner should the platform PC become unresponsive.
        ...
        Examples of such devices are interface adapters, motor controllers, wireless controllers, lidar, and encoders.
        ...
        For instance, as figure \ref{fig:system_diagram_software} shows, one node (`Ackermann kinematics') transforms steering input data to individual wheel velocities and positions based on Ackermann calculations.
...
\subsection{Sensor Selection}
    ...
    Selection factors were strengths and weaknesses in the context of orchard use, reported usage in literature, and availability at a sensible price.
...
% \subsection{Data Collection and Inspection}
 %    The sensors that seemed important to test, based on both the literature survey and the cost-benefit analysis were lidar, cameras and GPS.
    ...
    \subsubsection{In-orchard GPS Evaluation}
        ...
        Based on these results, we consider GPS to be unsuitable for use in kiwifruit orchards.
    ...
    \subsubsection{In-orchard Camera Evaluation}
        ...
        Overall, traditional 2D camera images were deemed to be the most suited for object detection and classification.
...
\subsection{Sensor Demonstration by Prototype Algorithm}
    ...
    \subsubsection{Object Detection and Classification}
        ...
        Each image was then converted to a indexed colour format.
        ...
        \begin{figure}[htb]
            ...
            \caption{
                An inference result from the FCN-8 network, segmenting a kiwifruit orchard row.
            }
            \label{fig:semSegRowResults}
        \end{figure}

        Initially, individual trees and posts in the kiwifruit orchard were labeled.
    	It was sound found that labeling the entire tree-line as a single class gave more robust results.
    ...
    \subsubsection{Mapping and Localisation}
        An existing Simultaneous Localisation and Mapping (SLAM) package was used to test the multi-layer lidar (Velodyne VLP-16).
        ...
        As the lidar has 16 scanning layers, a conversion was necessary to produce the single plane of lidar data required by Gmapping.
        ...
        The resulting points are then fed into Gmapping as a single plane along with the platform's wheel encoder data.
    ...
    \subsubsection{Kiwifruit Orchard Row Tracking}
        ...
        Using filtering technique used for SLAM mapping, a row guidance system is developed and tested.
        This algorithm also makes use of the on-board wheel encoders.
        Details of the algorithm used for this are published separately \citep{Bell2016}.
        ...
        This was necessary due to the lidar being mounted offset to the centreline of the platform \color{red}is this right?\color{black}.
...
\subsection{Sensor Selection Conclusions}
    ...
    Further research for using this as a means of generating a path for row following is underway and shall be published in future.
...
\label{sect:autonomous}
    ...
    This algorithm has been used to autonomously navigate over \SI{20}{\kilo\meter} of kiwifruit orchards with this platform and two smaller test platforms.
...
The authors acknowledge contributions from Phillip Ross, Gordon Neshausen, Josh Barnett, and Erin Simms who were involved with the design and fabrication of the platform, and Nicky Penhall for assistance training neural networks.
