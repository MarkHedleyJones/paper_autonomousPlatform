...
\label{sect:review}
    ...
    \subsection{Purpose-built Autonomous Vehicles in Agriculture}
        ...
        While its appearance resembled that of a computer server rack on wheels, it contained most of the components required on a platform for kiwifruit harvesting and pollination modules.
    ...
    \subsection{Sensors for Row Based Navigation in Orchards}
        ...
        These reports two navigation sensor approaches as being suitable for use in orchards.
        The use of lidar saw two of the reported vehicles navigate autonomously through orchard environments, which is encouraging.
        When combining cameras with neural network based processing, robustness to environmental complexities such as light or clutter was improved.
        ...
        Based on the increased reception requirements, we discount the use of an RTK based systems, but still consider the use of GNSS as a secondary sensor in-orchard.
...
\label{sect:design}
    ...
    \label{sub:steering}
        ...
        Implementing a four-wheel steering system would shift the pivot point to the vehicle's centre, roughly halving the radius.
    ...
    \label{sub:drive}
        ...
        A front pivoting axle ensures that all wheels remain in contact with the ground.

        % Estimation of the required torque is calculated by the following equations:
        Calculation of the drive system requirements for during acceleration up-hill is as follows:
        \begin{align}
        \label{eqn_f_rolling}
    ...
    \label{sect:architecture}
        ...
        Figure \ref{fig:system_diagram} illustrates how these devices are linked.
        ...
        All inputs from the joystick are also broadcast on the CAN bus, which are read by a software node on the Platform PC.
        ...
        This forces them to accept drive commands via CAN interface, allowing them to be directly controlled from ROS nodes.
        ...
        When this happens, all relays return to their `normal' states causing the motor controllers and on-board power supplies to be shut-off.
        Because power is required to disengage the park brakes, this also engages the brakes.
        Heartbeat message monitoring is a failsafe mechanism that ensures the system is automatically shut down in the event the Platform PC fail.
...
\label{sect:sensors}
    ...
    \subsubsection{In-orchard Camera Evaluation}
        ...
        \color{blue}\st{This was likely due to excessive ambient light and low reflectivity of objects in the orchard rows.}
        \color{black}
        This is though to be the result of two factors.
        ...
        This appeared to be the result of ambient sunlight interferring with the infrared projector.
        ...
        Latency of \SI{150}{\milli\second} was recorded when retrieving images form this camera.
...
\label{sect:autonomous}
    ...
    If the lidar system detects an object in the path of the vehicle during a turn, the steering is adjusted to avoid the object.
...
\label{sub:testing}
    ...
    The platform was routinely able to navigate two test orchards using the described approach.
    ...
    After tuning of the row end turn maneuvers, the platform successfully navigated the first block consecutively 7 times.
    ...
    Row end turning was a manual process that involved observing an autonomously driven turn and subsequently adjusting the length or radius of the turn's segments.
    Once tuned, the row end turns were robust \color{red} Need to quantify this\color{black}.
...
\label{sect:discussion}
    ...
    Signs of premature wear in the gearboxes of the steering motors has become apparent.
    These gearboxes are used to support frontal half of the vehicle's weight axially.
