...
\label{sect:autonomous}
    ...
    The smaller platforms used a skid-steer or differential-drive geometry, whereas our platform uses Ackermann steering geometry.
    This means that when turning, a skid/differential drive platform pivots along a lateral axis passing midway between the front and rear wheels.
...
\label{sub:testing}
  ...
  \subsection{Row Following}
    ...
    The process involved a recursive walk around the filtered points to search for squares with vertices defined by posts/trunks; a pair on the left-hand boundary and a pair on the right.
    Those squares were evaluated using a cost function to sort for those having a high degree of symmetry, regular internal angles, and appropriate length of sides,
    A similar process was then repeated to join the squares into chains that represented a continuous section of row.
    At each stage, the squares, and chain proposals were evaluated using cost functions to weigh-up strong proposals and filter-out weak proposals based on minimum acceptable criteria.
